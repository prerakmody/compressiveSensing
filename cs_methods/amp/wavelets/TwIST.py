...
def TwIST_raw(
    y, A, tau, psi_function=None, phi_function=None, lam1=1e-4, alpha=0, beta=0, AT=None,
    stop_criterion=1, tolA=0.01, debias=0, tolD=0.001,
    maxiter=1000, miniter=5, maxiter_debias=200, miniter_debias=5,
    init=0, enforce_monotone=True, sparse=True, true_x=None,
    verbose=True
    ):
    """
        Two-step Iterative Shrinkage/Thresholding Algorithm for Linear Inverse Problems.
    ...
    #
    if not isFunction(A):
        print ('A is not a function. Creating lamdas ... ')
    ...
    if np.isscalar(init):
        if init == 0:
            # initialize at zero, using AT to find the size of x (default)
    ...
    while cont_outer:
        ...
        while True:
            ...
            if (IST_iters >= 2) or (TwIST_iters != 0):
                ...
                # TwIST estimate (two-step iteration)
                ...
                if f > prev_f:
                    # if monotonicity  fails here  is  because max eig (A'A) > 1. Thus, we increase our guess of max_svs
                    max_svd *= 2
                    ...
                    TwIST_iters += 1
                    break # break while loop if curr_obj > prev_obj
        ...
        if stop_criterion == 0:
            # compute the stopping criterion based on the change of the number of non-zero components of the estimate
            criterion = num_changes_active
        elif stop_criterion == 1: 
            # compute the stopping criterion based on the relative variation of the objective function. (default)
        ...
        if verbose and iter % 10 == 0:
            pSNR = compare_psnr(x.astype('float32'), true_x.astype('float32'), dynamic_range=255)
            if plot_ISNR:
                print ('Iteration=%4d, ISNR=%4.5e  objective=%9.5e, nz=%7d, criterion=%7.3e' % \
                      (iter, pSNR, f, num_nz_x, criterion/tolA))
                # print ('Iteration=%4d, ISNR=%4.5e  objective=%9.5e, nz=%7d, criterion=%7.3e' % \
    ...
    if compute_mse: 
        mses_1 = np.array(mses) / true_x.size
