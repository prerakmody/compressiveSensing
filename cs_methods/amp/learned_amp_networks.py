...
import learned_amp_shrinkage as shrinkage
...
def build_LISTA(prob,T,initial_lambda=.1,untied=False):
    ...
    xhat_ = eta( By_, lam0_)  #with L-ISTA, there is a simple thresholding function and lambda is the parameter
    layers.append( ('LISTA T=1',xhat_, (lam0_,) ) ) # Layer structure (name, output, learnable params)
    
    for t in range(1,T):
        lam_ = tf.Variable( initial_lambda,name='lam_{0}'.format(t) )
        xhat_ = eta( tf.matmul(S_,xhat_) + By_, lam_ )    #thresh-func(Sx^ + By; λ)
...
def build_LAMP(prob,T,shrink,untied):
    ...
    return a list of layer info (name,xhat_,newvars)
     ...
     newvars : a tuple of layer-specific trainable variables
    """
    eta,theta_init = shrinkage.get_shrinkage_function(shrink)   #need to figure out a good shrinkage function for images
    ...
    ## LAYER 0: By (in first layer residual = y)
    B = A.T / (1.01 * la.norm(A,2)**2)
    B_ =  tf.Variable(B,dtype=tf.float32,name='B_0')
    By_ = tf.matmul( B_ , prob.y_ )
    layers.append( ('Linear',By_,None) )     #Linear Layer = B.y = (A.T/|A|) * y

    print ('\n2. Original theta:', theta_init)
    if getattr(prob,'iid',True) == False:
        # set up individual parameters for every coordinate
        theta_init = theta_init*np.ones( (N,1),dtype=np.float32 )
        print ('2. Final theta:', theta_init)
    ...
    (xhat_,dxdr_) = eta( By_,rvar_ , theta_ )  #thresh_func(r, eta' ,Θ) --> notice here that r = By, instead of r = x^ + By, since X^(0th) = [0] (zero vector)
    print ('2. Eta function returns: xhat', xhat_, ' dxdr_', dxdr_)
    layers.append( ('LAMP-{0} T=1'.format(shrink),xhat_,(theta_,) ) )

    vt_ = prob.y_   #residual
    for t in range(1,T):
        if len(dxdr_.get_shape())==2:
            dxdr_ = tf.reduce_mean(dxdr_,axis=0)
        
        # Step1 : Calculate the residual (with Onsager correction)
        bt_ = dxdr_ * NOverM
        vt_ = prob.y_ - tf.matmul( prob.A_ , xhat_ ) + bt_ * vt_    ## residual(t) = y - Ax^ + bt*residual(t-1);  bt = (N/M)*eta'
        
        # Step2 : Using the new residual, calulate a new x^
        ...
        if untied:
            ...
            rhat_ = xhat_ + tf.matmul(B_,vt_)   #eta(rhat; eta', Θ)
